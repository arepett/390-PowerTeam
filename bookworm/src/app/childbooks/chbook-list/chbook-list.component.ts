...
import { ChbookService } from 'src/app/chbook.service';

@Component({
  selector: 'app-chbook-list',
  template: `

    <main class="bg-img"><h1>CHILDRENS BOOKS</h1></main>
...
export class ChbookListComponent implements OnInit {
  books!: any;

  constructor(private chbookService: ChbookService) { }

  ngOnInit(): void {
    this.books = this.chbookService.getBooks();
      // .subscribe(books => {
      //   console.log(books);
      //   this.books = books;
      // });
  ...
  randomizer() {
    var titles = ['A Tale of Dark and Grimm', 'Jennifer L. Holm and Matthew Holm', 'Calendar Mysteries #1: January Joker', 'D.W. - Flips', 'Eerie Elementary #1: The School is alive', 'Fablehaven', "Galactic Hot Dogs #1: Cosmoe's Wiener Getaway",'Haggis and Tank Unleashed #1: All Paws on Deck'] 
